...
- Keep one folder per TeX note at repo root, named after the note basename (for example, `/notes/example/` for `example.tex`).
- Store the note source and all generated artifacts in that folder (`.tex`, `.aux`, `.log`, `.fls`, `.fdb_latexmk`, `.out`, `.synctex.gz`, `.bbl`, `.blg`, `.run.xml`, `-blx.bib`).
